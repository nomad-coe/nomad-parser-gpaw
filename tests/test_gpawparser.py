...
def test_gpw(parser):
    ...
    assert sec_method.basis_set[0].cell_dependent.kind == 'real space grid'
    assert sec_method.basis_set[0].cell_dependent.name == 'GR_20900.5'
...
def test_gpw2(parser):
    ...
    sec_method = archive.run[0].method[0]
    assert sec_method.basis_set[0].cell_dependent.kind == 'plane waves'
    assert sec_method.basis_set[0].cell_dependent.planewave_cutoff.magnitude == approx(4.8065299e-17)
...
def test_lcao(parser):
    ...
    assert archive.run[0].method[0].basis_set[0].atom_centered.kind == 'numeric AOs'
